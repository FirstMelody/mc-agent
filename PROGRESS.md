...
## 2026-09-23（傍晚）：两项增强的收口状态（诚实版）

**已实现且验证的**：鱼骨挖矿控制器（`start_mining(mode=branch, y=…)`）。隔离服
`MCAGENT_BRANCHMINE_TEST` PASS：1 次规划调用开局 → 主巷道+交替分支、透视找到两格石头后的铁矿并
采掘、耐久低问 Jev 一次（KEEP_MINING 生效）、**全程 0 次规划调用**；Jev 答 ESCALATE_LLM 时
**恰好 1 次**回退并把行程交回。已热部署生产 `23a01d7bc5636a72`（新问句默认 `interrupts=shadow`，
只记录不动手）。回归 TUNNELTEST / FARMTEST PASS。

**已实现但只验证了一半的**：作物成熟度 → Jev → todo。隔离服已证：田地目标能在"体况守护"下存活、
运行时扫描能看到作物（`crops=1`）、**收割本身零规划调用**（`harvested=1`）。未证：成熟度问句、
`TODO_LATER` 入队、空闲时段零调用执行——卡在 harness 的"让机器人忙起来"这一步：`mineAsTool`
在这个场景里始终没有真正起 job（`mining=false queue=0`），于是农场技能总能在问句之前把作物收走。
下一步是换一种占用方式（例如一个远处 `goto`）或先读 `mineAsTool` 的失败原因。

**完全未做的**：用户在本轮追加的两项需求——① 再次使用同一矿道时要寻找新路径（现在仍可能被历史
坏工作面拖住）；② 遇到开放空间（洞穴/峡谷）的确定性处理（现在只是拒绝→最多转两次 heading→
结束行程）。以及把"全程无调用"的断言从采掘阶段延长到回程。
